...
const getAllWorkFromDatabase = (minionId) => {
    const work = findDataArrayByName('work');
    return work.data.filter((element) => {
...
const updateWorkInstanceInDatabase = (instance, minionId) => {
    const model = findDataArrayByName('work');
    const instanceIndex = model.data.findIndex((element) => {
        return element.id === instance.id && element.minionId === minionId;
    });
    if (instanceIndex > -1 && model.isValid(instance)) {
        model.data[instanceIndex] = instance;
        return model.data[instanceIndex];
